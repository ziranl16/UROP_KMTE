...
class HyTE(Model):
        ...
        for i in range(len(posh)):
            if self.start_idx['triple'][i] < self.end_idx['triple'][i]:
                for j in range(self.start_idx['triple'][i] + 1, self.end_idx['triple'][i] + 1):
                    ...
                    if int(present_temp) - int(start_temp) >= 0 and str(
                            int(present_temp) - int(start_temp)) + ".0" in self.km_dict.keys():
    ...
    def __init__(self, params):
        self.reading()
        self.km_dict = loadK_KM_Estimate()
        self.p = params
